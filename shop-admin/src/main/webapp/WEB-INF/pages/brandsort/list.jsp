...
<script>
    ...
    function initTable() {
        $table.bootstrapTable({
            url: '<%=basePath%>brandsort/brandSortList.do',
            //height: getHeight(),
            locale: 'zh-CN',
            striped: true,
            //multipleSearch: true,
    ...
    function loadSpu(){
        $.ajax({
            url: '<%=basePath%>brandsort/listBrand.do',
            success: function (listAll){
                listAll = window.eval('('+listAll+')');
                brands = listAll['brandList'];if(brands.length == 0){
                    $.gritter.add({
                        title: '温馨提示',
                        text: '已经没有可添加的品牌了！',
    ...
    $('#submitBrandForm').on('click', function(){
        ...
        $.ajax({
            url: '<%=basePath%>brandsort/addBrand.do',
            data: {id:brandId},
            success: function(result){
                result = window.eval('('+result+')');
