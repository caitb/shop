...
<body class="no-skin" style="margin-top: 45px;">
                                                    ...
                                                    <form id="spuMainForm">
                                                        <div class="profile-user-info profile-user-info-striped">
                                                            ...
                                                            <div class="profile-info-row">
                                                                <div class="profile-info-name"> 家族 </div>
...
<script>
    ...
    function initTable() {
        $table.bootstrapTable({
            url: '<%=basePath%>familysort/familySortList.do',
            //height: getHeight(),
            locale: 'zh-CN',
            striped: true,
            //multipleSearch: true,
                    ...
                    },
                    {
                        ...
                        events: {
                            'click .delete': function(e, value, row, index){
                                $.ajax({
                                    url: '<%=basePath%>brandsort/deleteBrand.do',
                                    data:{id:row.id},
                                    success: function (){
        ...
        $table.on('editable-save.bs.table', function(e, fieId, row){
            $.ajax({
                url: '<%=basePath%>brandsort/updateSort.do',
                type: 'post',
                data: {id: row.id, sort: row['sm.sort']},
    ...
    function loadSpu(){
        $.ajax({
            url: '<%=basePath%>brandsort/listFamily.do',
            success: function (listAll){
                listAll = window.eval('('+listAll+')');
                familys= listAll['familyList'];
                if(familys.length == 0){
                    $.gritter.add({
                        title: '温馨提示',
                        text: '已经没有可添加的家族了！',
                ...
                for(var i in familys){
                    options += '<option value="'+familys[i].id+'">'+familys[i].name+'</option>';
                    familys[i].spus = [];
                    var spus = listAll['spuList'];
                    for(var j in spus){
                        if(spus[j].brandId == familys[i].id){
    ...
    $('#submitSpuMainForm').on('click', function(){
        ...
        $.ajax({
            url: '<%=basePath%>brandsort/addFamily.do',
            data: {id:brandId},
            success: function(result){
                result = window.eval('('+result+')');
