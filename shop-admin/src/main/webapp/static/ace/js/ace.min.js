...
ace.handle_side_menu = function ($) {
    ...
    $.ajax({
        ...
        success: function(data){
            ...
            var nodes = zTree.transformTozTreeNodes(data);
            ...
            $(".nav-list").on(ace.click_event, function (e) {
                ...
                if(!$ul.hasClass('submenu')){
                    $('.nav-list b').removeClass('icon-angle-down').addClass('icon-angle-right');
                    $(".nav-list ul:visible").toggle(200);
                }

                var $a = $(e.target).closest('a');
                var $submenu = $($a.next().get(0));
                if($submenu.is('ul')){
                    $submenu.slideToggle(200, function(){
                        console.log('b--: ' + $submenu.css('display'));
                        $submenu.css('display')=='block' ? $a.children('b').removeClass('icon-angle-right').addClass('icon-angle-down') : $a.children('b').removeClass('icon-angle-down').addClass('icon-angle-right');
                    });
                }else{
                    $a.parents('li').addClass('active').addClass('open');
                    var _link = '<li><i class="icon-home home-icon"></i><a href="#">首页</a></li>';
                    $('.nav-list .active').each(function(i, obj){
                        _link += '<li>' + $(obj).children('a').find('span').html() + '</li>';
                    })
                    $('.breadcrumb').html(_link);
                    $.ajax({
                        url: $a.attr('href'),
                        success: function(data){
