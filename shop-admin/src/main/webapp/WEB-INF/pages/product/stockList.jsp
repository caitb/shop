                ...
                [
                    ...
                    },
                    {
                        ...
                        formatter: function(value, row, index){
                            if(row.pfSkuStock && row.pfSkuStock.id){
                    ...
                    },
                    {
                        ...
                        align: 'center',
                        formatter: function(value, row, index){
                            if(row.pfSkuStock && row.pfSkuStock.isQueue == 0){
                                return '<a class="queue" href="javascript:void(0);">进入排单</a>';
                            }
                            if(row.pfSkuStock && row.pfSkuStock.isQueue == 1){
                                return '&nbsp;&nbsp;<a class="queue" href="javascript:void(0);">结束排单</a>';
                            }
                        },
                        events: {
                            'click .queue': function(e, value, row, index){
                                var message = row.pfSkuStock.isQueue==0 ? "您确认进入排单?<br/>商品进入排单后，系统会记录用户付款的时间，发货时按付款的先后顺序发货" : '您确认要结束排单?';
                                    ...
                                    if(result) {
                                        $.ajax({
                                            ...
                                            success: function(msg){
                                                ...
                                                $.gritter.add({
                                                    title: '消息',
                                                    text: msg=='success' ? (row.pfSkuStock.isQueue==0 ? '此商品已进入排单状态!' : '此商品已结束排单!') : '进入排单失败!',
        ...
        $table.on('editable-save.bs.table', function(e, fieId, row){
            $.ajax({
                ...
                success: function(msg){
                    $.gritter.add({
                        title: '消息',
                        text: msg=='success' ? '库存更新成功!' : '库存更新失败!',
                        class_name: 'gritter-success' + (!$('#gritter-light').get(0).checked ? ' gritter-light' : '')
                    });
                }
            })
