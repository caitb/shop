...
<script>
    ...
    function initTable() {
        $table.bootstrapTable({
            url: '<%=basePath%>banner/bannerList.do',
            //height: getHeight(),
            locale: 'zh-CN',
            striped: true,
            //multipleSearch: true,
            ...
            columns: [
                [
                    ...
                    {
                        field: 'banner.hyperlink_url',
                        title: 'hyperlinkUrl',
                        sortable: true,
                        footerFormatter: totalNameFormatter,
                        align: 'center',
                        formatter: function(value, row, index){
                            return row.hyperlink_url;
                        }
                    },
                    {
                        field: 'banner.img_url',
                        title: 'imgUrl',
                        sortable: true,
                        footerFormatter: totalNameFormatter,
                        align: 'center',
                    ...
                    },
                    {
                        ...
                        formatter: function(value, row, index){
                            var arr = [];
                                arr.push('<a class="delete" href="javascript:void(0);">删除内容</a>');
                                arr.push('<a class="edit  href="javascript:void(0);">编辑内容</a>');
                            return arr.join(' ');
    },
                        events: {
                            'click .delete': function(e, value, row, index){
                                $.ajax({
                                    url: '<%=basePath%>banner/deleteBanner.do',
                                    data:{id:row.id},
                                    success: function (){
                            ...
                            'click .edit': function(e, value, row, index){
                                parent.window.$('#myTabbable').closeTab('edit-banner');
                                parent.window.$('#myTabbable').add('edit-banner', '编辑banner', '<%=basePath%>banner/editBanner.shtml?id='+row.id);
                            }
                        }
                    }
                ]
