...
<script>
    ...
    function initTable() {
        $table.bootstrapTable({
            locale: 'zh-CN',
            url: '<%=basePath%>materielApplication/list.do',
            striped: true,
            //multipleSearch: true,
            ...
            columns: [
                [
                    ...
                    },
                    {
                        ...
                        formatter: function(value, row, index){
                            if(row.status == 0){
                                return '<a class="send" href="javascript:void(0);">发送</a>';
                            }else if(row.status == 1){
                                return '已发送';
                            }
                        },
                        events: {
                            'click .send': function(e, value, row, index){
                                bootbox.confirm('您确认已经发送线下物料到该用户的邮箱?', function(result) {
                                    if(result) {
                                        $.ajax({
                                            url: '<%=basePath%>materielApplication/update.do',
        ...
        $table.on('editable-save.bs.table', function(e, fieId, row){
            $.ajax({
                url: '<%=basePath%>material/updateLibrary.do',
                type: 'post',
