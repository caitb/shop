...
public class PfBorderService {
    ...
    public void cancelUnPayUpgradeBOrder(PfBorder bOrder) {
        try {
            ...
            bOrder = borderMapper.selectByPrimaryKey(bOrder.getId());
            // 检查订单状态的有效性
            if (bOrder.getOrderStatus().intValue() != BOrderStatus.NotPaid.getCode().intValue()) {
                throw new BusinessException("订单状态不正确,订单号:" + bOrder.getOrderCode()
                        + ",当前订单状态为:" + bOrder.getOrderStatus());
            }
            if (bOrder.getPayStatus().intValue() != 0) {
                throw new BusinessException("订单支付状态不正确,订单号:" + bOrder.getOrderCode()
                        + ",当前订单支付状态为:" + bOrder.getPayStatus());
            }
            if (bOrder.getOrderType().intValue() != BOrderType.UPGRADE.getCode().intValue()) {
                throw new BusinessException("订单类型不正确,订单号:" + bOrder.getOrderCode()
                        + ",当前订单类型是:" + bOrder.getOrderType());
            }
            log.info("订单状态,支付状态,订单类型校验通过!");
            // 取消订单
            // 修改订单的状态为已取消状态
            int result = borderMapper.updateCancelByIdAndOStatusAndPStatusAndOType(bOrder.getId(),
                    BOrderStatus.NotPaid.getCode(), 0, BOrderType.UPGRADE.getCode());
            if (result != 1) {
                bOrder = borderMapper.selectByPrimaryKey(bOrder.getId());
                throw new BusinessException("订单取消失败,订单此时状态为:" + bOrder.getOrderStatus()
                        + ",支付状态为:" + bOrder.getPayStatus() + ",订单类型为:" + bOrder.getOrderType());
            }
            // 插入订单操作记录
            bOrderOperationLogService.insertBOrderOperationLog(bOrder,"超过2天未支付升级订单,系统自动取消");

            log.info("取消关联升级通知单");

            // 关联升级单取消
            PfUserUpgradeNotice notice = pfUserUpgradeNoticeService.findByPfBorderId(bOrder.getId());
            if(notice == null){
                throw new BusinessException("没有找到对应的关联升级通知单");
            }
            //pfUserUpgradeNoticeService.cancelUpgradeNotice(notice);
            // 所有下级升级单置为待支付
            // 微信通知所有有升级需求的代理商
        } catch (Exception e) {
