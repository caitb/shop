                                    ...
                                    <div id="toolbar">
                                        <div class="form-inline pull-right">
                                            <div class="form-group">
                                                <label for="beginTime">抽奖时间：</label>
                                            </div>
                                            <div class="form-group">
...
<script>
    ...
    function view(row) {
        var param = {
            turnTableId : row.turnTable.id,
            beginTime : Search.beginTime,
            endTime :   Search.endTime
        }
        $.ajax({
            url : '<%=basePath%>turnRule/detailStatistics.do',
            data : param,
            dataType : 'json',
            success : function (data) {
                var $detailDialog = $('#detailDialog');
                $detailDialog.find('tbody').text('');

                for(var i in data) {
                    var row = data[i];
                    var name = row.comGift.name;
                    var count = row.count;
                    var $row = '<tr class="row"> <td class="col-sm-5">'+name+'</td> <td class="col-sm-7">'+count+'</td> </tr>';
                    $detailDialog.find('tbody').append($row);
                }
    ...
    function initTable() {
        $table.bootstrapTable({
            url: '<%=basePath%>turnRule/listStatistics.do',
            //height: getHeight(),
            locale: 'zh-CN',
            striped: true,
            //multipleSearch: true,
            ...
            columns: [
                [
                    ...
                    },
                    {
                        ...
                        formatter: function (value, row, index) {
                            if(row.giftCount) {
                                return '<a href="javascript:void(0);" class="view">'+row.giftCount+'</a>';
                            }
                        },
                        events : {
        ...
        $('#searchBtn').on('click', function(){
            ...
            Search.beginTime = $('[name=beginTime]').val();
            Search.endTime =   $('[name=endTime]').val();
        });
    }
