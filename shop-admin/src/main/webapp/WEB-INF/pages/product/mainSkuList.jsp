                                ...
                                <div>
                                    <div id="toolbar">
                                        <div class="form-inline">
                                            <button type="button" class="btn btn-default" id="addBtn">添加主打商品</button>
                                        </div>
                                    </div>
                                ...
                                <div class="modal-content">
                                                ...
                                                <div class="col-xs-12 col-sm-12 col-sm-offset-0">
                                                    ...
                                                    <div class="profile-user-info profile-user-info-striped">
                                                        ...
                                                        <div class="profile-info-row">
                                                            <div class="profile-info-name"> 品牌名 </div>
                                                        ...
                                                        <div class="profile-info-row">
                                                            <div class="profile-info-name"> 设置主打商品 </div>

                                                            <div class="profile-info-value">
                                                                <select class="form-control" id="spuMainType" name="type">
                                                                </select>
                                                            </div>
                                                        </div>
                                    ...
                                    <div class="modal-footer no-margin-top">
                                        <div class="col-xs-5 col-sm-5 col-sm-offset-4">
                                            ...
                                            <button class="btn btn-sm btn-info pull-left ok" id="submitSpuMainForm">
                                                确认
                                            </button>
                                        </div>
                                    </div>
...
<script>
    ...
    function initTable() {
        $table.bootstrapTable({
            url: '<%=basePath%>product/mainSkuList.do',
            //height: getHeight(),
            locale: 'zh-CN',
            striped: true,
            //multipleSearch: true,
            ...
            columns: [
                [
                    ...
                    },
                    {
                        ...
                        formatter: function(value, row, index){
                            var arr = [];
                            arr.push('<a class="edit" href="javascript:void(0);">设置主打商品</a>');
                            arr.push('<a class="delete"  href="javascript:void(0);">删除</a>');
                            return arr.join(' ');
    },
                        events: {
                            'click .edit': function(e, value, row, index){
                                $.ajax({
                                    url: '<%=basePath%>product/listSpu.do',
                                    data:{brandId:row.brandId},
                                    success: function (selectByBrandId){
                                        selectByBrandId = window.eval('('+selectByBrandId+')');
                                        var options = '';
                                        for(var i in selectByBrandId){
                                            if(row.spuId == selectByBrandId[i].id){
                                                options += '<option value="'+selectByBrandId[i].id+'" selected>'+selectByBrandId[i].name+'</option>';
                                            }else{
                                                options += '<option value="'+selectByBrandId[i].id+'">'+selectByBrandId[i].name+'</option>';
                                            }
                                        }
                                        $('#brandId').html('<option value="'+row.brandId+'">'+row.cname+'</option>');
                                        $('#spuMainType').html(options);
                            ...
                            'click .delete': function(e, value, row, index){
                                $.ajax({
                                    url: '<%=basePath%>product/deleteMain.do',
    ...
    function loadSpu(){
        $.ajax({
            url: '<%=basePath%>product/listBrand.do',
            success: function (listAll){
                listAll = window.eval('('+listAll+')');
                brands = listAll['brandList'];
                ...
                var options = '';
                for(var i in brands){
                    options += '<option value="'+brands[i].id+'">'+brands[i].cname+'</option>';

                    brands[i].spus = [];
                    var spus = listAll['spuList'];
                    for(var j in spus){
                        if(spus[j].brandId == brands[i].id){
                            brands[i].spus.push(spus[j]);
                        }
                    }
                }

                var option = '';
                for(var j in brands[0].spus){
                    option += '<option value="'+brands[0].spus[j].id+'">'+brands[0].spus[j].name+'</option>';
                }
                $('#brandId').html(options);
                $('#spuMainType').html(option);
                $('#modal-add').modal('show');
            }
        });
    }
    ...
    $('#brandId').change(function(){
        ...
        for(var i in brands){
            if(brandId == brands[i].id){
                for(var j in brands[i].spus){
                    options += '<option value="'+brands[i].spus[j].id+'">'+brands[i].spus[j].name+'</option>';
                }
            }
        }
        $('#spuMainType').html(options);
    ...
    $('#submitSpuMainForm').on('click', function(){
        var brandId = $('#brandId').val();
        var spuId = $('#spuMainType').val();
        ...
        $.ajax({
            url: '<%=basePath%>product/addSpuMain.do',
            data: {brandId: brandId, spuId: spuId, type: 0},
            success: function(result){
                result = window.eval('('+result+')');
