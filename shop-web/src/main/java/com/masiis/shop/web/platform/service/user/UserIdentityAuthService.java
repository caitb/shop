...
public class UserIdentityAuthService {
    ...
    public int sumbitAudit(HttpServletRequest request, ComUser comUser,String idCardFrontName,String idCardBackName,Integer type){
        try{
            ...
            //修改用户数据
            if (comUser.getIdCardFrontUrl()==null||(comUser.getIdCardFrontUrl()!=null&&!comUser.getIdCardFrontUrl().equals(idCardFrontName))){
                //临时目录copy到正式目录
            ...
            if (comUser.getIdCardBackUrl()==null||(comUser.getIdCardBackUrl()!=null&&!comUser.getIdCardBackUrl().equals(idCardBackName))){
                //临时目录copy到正式目录
