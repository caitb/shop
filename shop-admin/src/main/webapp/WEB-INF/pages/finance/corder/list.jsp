...
<script>
    ...
    function initTable() {
        $table.bootstrapTable({
            url: '<%=basePath%>finance/corder/list.do',
            //height: getHeight(),
            locale: 'zh-CN',
            striped: true,
            //multipleSearch: true,
            ...
            columns: [
                [
                    ...
                    },
                    {
                        ...
                        formatter: function(value, row, index){
                            if(row.pfCorder){
                                return row.pfCorder.receivableAmount;
                            }
                        }
                    },
                    {
                    ...
                    },
                    {
                        ...
                        formatter: function(value, row, index){
                            ...
                            if(row.pfCorder && row.pfCorder.orderStatus == 7){
                                return '待发货';
                            }
                            if(row.pfCorder && row.pfCorder.orderStatus == 8){
                                return '已发货';
                            }
                        }
                    },
                    {
                        ...
                        align: 'center',
                        formatter: function(value, row, index){
                            if(row.pfCorderPayments){
                                var sHtm = '';
                                for(var i in row.pfCorderPayments){
                                    if(row.pfCorderPayments[i].payTypeId == 0) sHtm += '微信支付';
                                }
                                return sHtm;
                            }
                        }
                    },
                    {
                        ...
                        align: 'center',
                        formatter: function(value, row, index){
                            if(row.pfCorder && row.pfCorder.payStatus == 0){
                                return '待付款';
                            }
                            if(row.pfCorder && row.pfCorder.payStatus == 1){
                                return '已付款';
                            }
                        }
                    },
                    {
                        ...
                        formatter: function(value, row, index){
                            ...
                            if(row.pfCorder && row.pfCorder.shipStatus == 5){
                                return '已发货';
                            }
                            if(row.pfCorder && row.pfCorder.shipStatus == 9){
                                return '已收货';
                            }
                        }
                    },
                    {
                        ...
                        align: 'center',
                        formatter: function(value, row, index){
                            if(row && row.pfCorderFreights && row.pfCorderFreights[0] && row.pfCorderFreights[0].shipManName){
                                return row.pfCorderFreights[0].shipManName;
                            }
                        }
                    },
                    {
                        ...
                        align: 'center',
                        formatter: function(value, row, index){
                            if(row && row.pfCorderFreights && row.pfCorderFreights[0] && row.pfCorderFreights[0].freight){
                                return row.pfCorderFreights[0].freight;
                            }
                        }
                    },
                    {
