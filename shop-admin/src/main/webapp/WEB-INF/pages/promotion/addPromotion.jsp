    ...
    $(document).ready(function() {
        ...
        function loadPromotion(promotionId) {
            ...
            function viewPromotion(promotion) {
                $('input[name=promotionId]').val(promotion.id);
                $('input[name=promotionName]').val(promotion.name);
                $('input[name=promotionRemark]').val(promotion.remark);
                $('[name=promotionPersonType]').val(promotion.personType);
            ...
            function viewRuleWraps(ruleWraps) {
                $('.promotion-rule:visible').remove();

                for(var i in ruleWraps) {
                    var rule = ruleWraps[i].rule;
                    var promotionGift = ruleWraps[i].promotionGift;

                    var $template = $('#promotion-rule-template');
                    var $newRule = $template.clone(true).removeAttr("id").show();

                    $newRule.find('[attr=ruleId]').val(rule.id);
                    $newRule.find('[attr=ruleValue]').val(rule.ruleValue);

                    $newRule.find('[attr=promotionGiftId]').val(promotionGift.id);
                    $newRule.find('[attr=giftValue]').val(promotionGift.giftValue)
                    $newRule.find('[attr=quantity]').val(promotionGift.quantity);
                    $newRule.find('[attr=upperQuantity]').val(promotionGift.upperQuantity);
            ...
            $.ajax({
                url : "<%=basePath%>promotion/getPromotion",
                data : {'promotionId': promotionId},
                dataType : 'json',
                async : true,
                success : function(data) {
                    console.log(data);
                    viewPromotion(data.promotion);
                    viewRuleWraps(data.ruleWraps);
                }
            })
