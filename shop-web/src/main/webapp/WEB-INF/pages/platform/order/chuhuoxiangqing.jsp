    ...
    <main>
        <div id="box">
            <div class="sec1"><c:if test="${borderDetail.pfBorder.payStatus==1 &&borderDetail.pfBorder.shipStatus==0}">
                ...
                <h1>未付款</h1>
                <p>亲，订单还未处理~~</p></c:if><c:if
                    test="${borderDetail.pfBorder.payStatus==1 &&borderDetail.pfBorder.shipStatus==5}">
                <img src="<%=path%>/static/images/icon_68.png" alt=""
                     style="display: block;width: 56px;height: 30px;top: 14px;left: 7px;">
                <h1>未收货</h1>
                <p>亲，订单还未完结~~</p></c:if><c:if
                    test="${borderDetail.pfBorder.orderStatus==3 &&borderDetail.pfBorder.shipStatus==9}">
            ...
            <div class="sec2">
                ...
                <p><span>下单日期：</span><span><fmt:formatDate value="${borderDetail.pfBorder.createTime}"
                ...
                <p>类　　型：<span><c:if test="${borderDetail.pfBorder.orderType==0}">下级合伙订单</c:if><c:if test="${borderDetail.pfBorder.orderType==1}">下级补货</c:if><c:if test="${borderDetail.pfBorder.sendType==1 && borderDetail.pfBorder.orderType==2}">申请拿货</c:if></span></p>
                ...
                <p><span>发货时间：</span><span><fmt:formatDate value="${borderDetail.pfBorder.shipTime}"
                                                           pattern="yyyy-MM-dd HH:mm"/></span></p><c:forEach
                    items="${borderDetail.pfBorderFreights}" var="bpf"><c:if test="${not empty bpf.freight}">
                ...
                <section class="dizhi">
                    ...
                    <div>
                        <a href="#"><h2>收货人：<b>${borderDetail.pfBorderConsignee.consignee}</b>
                            <span>${borderDetail.pfBorderConsignee.mobile}</span></h2></a>
                        <a href="#"><p>收货地址：
                            <span>${borderDetail.pfBorderConsignee.provinceName} ${borderDetail.pfBorderConsignee.cityName} ${borderDetail.pfBorderConsignee.regionName} ${borderDetail.pfBorderConsignee.address}</span>
                        </p></a>
                    </div>
                </section>
            ...
            <div class="sec4"><c:forEach items="${borderDetail.pfBorderItems}" var="bdpd">
                ...
                <p><span>订单状态：</span><c:if test="${borderDetail.pfBorder.orderStatus==0}"><span>未处理</span></c:if><c:if
                        test="${borderDetail.pfBorder.orderStatus==1}"><span>已付款</span></c:if><c:if
                        test="${borderDetail.pfBorder.orderStatus==3}"><span>成功交易</span></c:if></p>
            </div>
        </div>
    </main>
    <div class="back_que">
